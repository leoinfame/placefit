...
// ─── Gerar HTML do pedido de compra (para PDF/WhatsApp) ──────────────────────
...
<style>
  *{box-sizing:border-box;margin:0;padding:0}
  ...
  .header{display:flex;gap:16px;border:1px solid #ccc;border-radius:6px;padding:12px;margin-bottom:16px}
  .header-side{flex:1;display:flex;flex-direction:column;gap:2px}
  .header-side h3{font-size:13px;font-weight:bold;margin-bottom:4px;color:#1e40af}
  .logo{width:48px;height:48px;object-fit:contain;border-radius:4px;margin-bottom:4px}
  .name{font-size:13px;font-weight:bold}
  .info{font-size:11px;color:#555;line-height:1.5}
  .divider{width:1px;background:#ddd}
  .ref-box{background:#f1f5f9;border-radius:6px;padding:8px 12px;margin-bottom:16px;font-size:12px}
  .ref-box span{font-weight:bold}
  table{width:100%;border-collapse:collapse;margin-bottom:16px}
  th{background:#1e40af;color:#fff;padding:8px;text-align:left;font-size:11px}
  td{border-bottom:1px solid #eee;padding:7px 8px;font-size:11px}
  tr:nth-child(even) td{background:#f9fafb}
  .total-row{background:#1e40af!important;color:#fff}
  .total-row td{color:#fff;font-weight:bold;font-size:13px;padding:10px 8px}
...
<div class="header">
  <div class="header-side">
    ...
    ${fabricante?.logomarca ? `<img src="${fabricante.logomarca}" class="logo" alt="logo">` : ""}
    <div class="name">${fabricante?.empresa || fabricante?.full_name || fabricante?.nome || "—"}</div>
    <div class="info">
      ${fabricante?.whatsapp ? `📱 ${fabricante.whatsapp}<br>` : ""}
      ${fabricante?.email ? `✉ ${fabricante.email}<br>` : ""}
      ${fabricante?.endereco ? `📍 ${fabricante.endereco}<br>` : ""}
      ${fabricante?.cnpj ? `CNPJ: ${fabricante.cnpj}` : ""}
    </div>
  </div>
  ...
  <div class="header-side">
    ...
    <div class="info">
      ${revendedor?.whatsapp ? `📱 ${revendedor.whatsapp}<br>` : ""}
      ${revendedor?.email ? `✉ ${revendedor.email}<br>` : ""}
      ${revendedor?.endereco ? `📍 ${revendedor.endereco}<br>` : ""}
      ${revendedor?.cnpj ? `CNPJ: ${revendedor.cnpj}` : ""}
    </div>
  </div>
</div>

<div class="ref-box">
  📦 Pedido de Compra referente à Venda: <span>${pedidoVenda.numero_pedido}</span>
  &nbsp;&nbsp;|&nbsp;&nbsp; Cliente final: <span>${pedidoVenda.cliente_nome || "—"}</span>
  &nbsp;&nbsp;|&nbsp;&nbsp; Data: <span>${fmtDate(pedidoVenda.data_pedido)}</span>
</div>

<table>
  <thead>
    <tr>
      <th>Código</th><th>Produto</th><th style="text-align:center">Qtd</th>
      <th style="text-align:right">Preço Unit.</th><th style="text-align:right">Subtotal</th>
    </tr>
  </thead>
  <tbody>
    ${itensRows}
...
export default function PedidosCompra() {
  ...
  const [fabricantes, setFabricantes] = useState([]); // lista de users fabricantes
  const [loading, setLoading] = useState(true);
  const [searchTerm, setSearchTerm] = useState("");
  const [selectedVenda, setSelectedVenda] = useState(null); // venda aberta
  const [previewPedido, setPreviewPedido] = useState(null); // { fabricante, itens }
  ...
  // Agrupar itens de uma venda por fabricante
  ...
  const handleWhatsApp = (venda, grupo) => {
    ...
    if (phone) {
      ...
      toast({ title: "WhatsApp", description: "Fabricante sem número cadastrado. Texto copiado para envio manual." });
    }
  };
  ...
  // ── Tela: detalhes de uma venda (pedidos por fabricante) ──────────────────
  if (selectedVenda) {
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-500">
                Cliente: <strong>{selectedVenda.cliente_nome}</strong> &nbsp;·&nbsp;
                Data: {fmtDate(selectedVenda.data_pedido)} &nbsp;·&nbsp;
                Total da Venda: <strong className="text-green-700">{fmtBRL(selectedVenda.total)}</strong>
              </p>
            </div>
          </div>
                ...
                <Card key={idx} className="shadow-sm border border-gray-200 overflow-hidden">
                  ...
                  <div className="flex flex-wrap items-center gap-2 px-4 py-3 bg-gray-50 border-t">
                    <Button
                      size="sm"
                      variant="outline"
                      onClick={() => setPreviewPedido({ venda: selectedVenda, grupo })}
                      className="gap-1"
                    >
                    ...
                    <Button
                      size="sm"
                      variant="outline"
                      onClick={() => handlePDF(selectedVenda, grupo)}
                    ...
                    <Button
                      size="sm"
                      onClick={() => handleWhatsApp(selectedVenda, grupo)}
        ...
        {previewPedido && (
          <Dialog open={!!previewPedido} onOpenChange={() => setPreviewPedido(null)}>
            <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
              <DialogHeader>
                <DialogTitle>
                  Pedido — {previewPedido.grupo.fabricante?.empresa || previewPedido.grupo.fabricante?.full_name || "Fabricante"}
                </DialogTitle>
              </DialogHeader>
              <div
                className="text-sm"
                dangerouslySetInnerHTML={{
                  __html: buildOrderHTML(
                    previewPedido.venda,
                    previewPedido.grupo.fabricante,
                    previewPedido.grupo.itens,
                    user
                  )
                }}
              />
    ...
    <div className="p-4 md:p-8 min-h-screen">
      <div className="max-w-5xl mx-auto space-y-6">
        ...
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">Selecione uma venda para visualizar os pedidos de compra por fabricante.</p>
        </div>
        ...
        <div className="relative">
          ...
          <Input
            placeholder="Buscar por número de pedido ou cliente..."
            value={searchTerm}
                    ...
                    <div className="flex items-center justify-between gap-4">
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-sm text-gray-600">
                          ...
                          <strong>Data:</strong> {fmtDate(venda.data_pedido)}
                        </p>
                        <p className="text-xs text-gray-400 mt-1">
                          {grupos.length} pedido(s) de compra &nbsp;·&nbsp; {(venda.itens || []).length} produto(s)
                        </p>
                      </div>
