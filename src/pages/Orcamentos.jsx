...
export default function Orcamentos() {
  ...
  const generatePDF = (orcamento) => {
    ...
    const htmlContent = `
      ...
      <head>
        <meta charset="UTF-8">
        <title>Orçamento ${orcamento.numero_pedido}</title>
        <style>
      ...
      <body>
        ...
        <div class="title">ORÇAMENTO - ${orcamento.numero_pedido}</div>
        <div style="text-align: center; font-size: 11px; color: #666; margin-bottom: 20px;">
          Data: ${new Date(orcamento.data_pedido).toLocaleDateString('pt-BR')}
        </div>
        ...
        <div class="section">
          ...
          <div style="font-size: 11px;">
            ...
            ${cliente?.email ? `E-mail: ${cliente.email}<br>` : ''}
            ${cliente?.endereco ? `Endereço: ${cliente.endereco}, ${cliente.cidade}/${cliente.estado}` : ''}
          </div>
        </div>
        ...
        <div class="section">
          <table>
            <tr>
              ...
              <td class="total-row text-right">R$ ${orcamento.subtotal.toFixed(2)}</td>
            </tr>
            <tr>
              <td>Frete</td>
              <td class="text-right">R$ ${orcamento.frete.toFixed(2)}</td>
            </tr>
            <tr>
              <td>Desconto</td>
              <td class="text-right">R$ ${orcamento.desconto.toFixed(2)}</td>
            </tr>
            <tr style="background-color: #e0e0e0; font-weight: bold; font-size: 13px;">
              <td>TOTAL</td>
              <td class="text-right">R$ ${orcamento.total.toFixed(2)}</td>
            </tr>
          </table>
        </div>
        ...
        ${orcamento.observacoes ? `
          <div class="section">
            ...
            <div style="font-size: 11px;">${orcamento.observacoes}</div>
          </div>
        ` : ''}
      </body>
      </html>
