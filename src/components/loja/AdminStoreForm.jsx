...
export default function AdminStoreForm({ resellers, config, onSaved }) {
  ...
  useEffect(() => {
    ...
    else setForm({ revendedor_id: "", slug: "", nome_loja: "", logo_url: "", banner_url: "", cor_primaria: "#1e40af", cor_secundaria: "#059669", descricao: "", frete_fixo_valor: 0, frete_gratis_valor: 0, aceita_pix: true, aceita_cartao: false, aceita_boleto: false, aceita_dinheiro: false, whatsapp_contato: "", ativo: false });
  }, [config]);

  if (!form) return null;

  const save = async () => {
    if (!form.revendedor_id || !form.slug || !form.nome_loja) { toast({ title: "Preencha revendedor, slug e nome", variant: "destructive" }); return; }
    setSaving(true);
    try {
  ...
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        ...
        <div><Label>WhatsApp de contato</Label><Input value={form.whatsapp_contato} onChange={(e) => setForm({ ...form, whatsapp_contato: e.target.value })} /></div>
      </div>
      ...
      {embed && (
        <div className="bg-gray-900 text-green-400 rounded-lg p-3 text-xs font-mono relative">
          ...
          <p className="text-gray-400 mt-2 font-sans">Cole este código no site do revendedor (WordPress etc.) para exibir a loja.</p>
        </div>
      )}
    </div>
  );
